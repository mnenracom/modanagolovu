...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      // Пробуем отправить запрос с настройками для надежного HTTPS соединения
      // Deno может иметь проблемы с некоторыми SSL сертификатами, поэтому добавляем дополнительные опции
      response = await fetch(apiUrl, {
        method: 'POST',
        headers: headers,
        body: JSON.stringify(paymentRequest),
        signal: controller.signal,
        // Настройки для надежного HTTPS соединения
        keepalive: true, // Включаем keepalive для переиспользования соединений
        // Deno автоматически обрабатывает SSL/TLS, но можем добавить дополнительные опции через переменные окружения
      })
